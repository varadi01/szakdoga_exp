...
from impl.scenario import Environment, Step, SimpleGame
...
class Individual:
    ...
    def __init__(self, generation: int, known_actions: ActionHolder = ActionHolder(), scenario: SimpleGame = SimpleGame):
        ...
        self.known_actions = known_actions #TODO rename 'moving function'
...
class IndividualNaive:
    ...
    def __init__(self, generation: int, moving_functions = [],  scenario: SimpleGame = SimpleGame):
        self.generation = generation
...
class Genetic:
    ...
    def reproduction(self):
        ...
        while len(self.generation) < POPULATION_SIZE:
            ...
            childs_inherited_actions: ActionHolder = ActionHolder()
            if random() < CROSSOVER_RATE:
                parent1, parent2 = parent2, parent1
            for action in parent1.known_actions.actions:
                childs_inherited_actions.add_action(action)
            for action in parent2.known_actions.actions:
                childs_inherited_actions.add_action(action)
            #create child
            new_individual = Individual(self.newest_generation_number, childs_inherited_actions)
